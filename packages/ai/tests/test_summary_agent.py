...
def test_summary_agent_returns_output_from_provider() -> None:
    ...
    out = asyncio.run(
        agent.execute(
            TenantId.new(),
            SummaryAgentInput(period_kind="weekly", start_date=date(2026, 8, 1), end_date=date(2026, 8, 7), entries=_entries()),
        )
    )
...
def test_summary_agent_persists_agent_run_with_version_hash() -> None:
    provider = FakeAIProvider(chat_response='{"title":"x","markdown":"y","top_learnings":[],"metrics":{}}')
    runs = FakeAgentRunRepository()
    agent = SummaryAgent(provider=provider, prompt_engine=PromptEngine(PROMPTS), runs=runs)
    ...
    out = asyncio.run(
        agent.execute(
            TenantId.new(),
            SummaryAgentInput(period_kind="daily", start_date=date(2026, 1, 1), end_date=date(2026, 1, 1), entries=[{"title": "x", "study_minutes": 30}]),
        )
    )
...
def test_summary_agent_handles_invalid_json_gracefully() -> None:
    ...
    out = asyncio.run(
        agent.execute(
            TenantId.new(),
            SummaryAgentInput(period_kind="monthly", start_date=date(2026, 1, 1), end_date=date(2026, 1, 31), entries=[]),
        )
    )
...
def test_summary_agent_sends_system_prompt_with_template_variables() -> None:
    ...
    asyncio.run(
        agent.execute(
            TenantId.new(),
            SummaryAgentInput(period_kind="weekly", start_date=date(2026, 1, 1), end_date=date(2026, 1, 7), entries=_entries()),
        )
    )
...
def test_summary_agent_passes_temperature_and_max_tokens() -> None:
    ...
    agent = SummaryAgent(provider=provider, prompt_engine=PromptEngine(PROMPTS), runs=runs, config=cfg)
    asyncio.run(
        agent.execute(
            TenantId.new(),
            SummaryAgentInput(period_kind="daily", start_date=date(2026, 1, 1), end_date=date(2026, 1, 1), entries=[]),
        )
    )
